...
int main(int argc, char* argv[])
{
  ...
  // auto node = std::make_shared<rclcpp::Node>("olei_driver");
  auto node = std::make_shared<rclcpp::Node>("olei_driver");
    std::string transport_str, scanner_ip, port, topic, frame_id, packet_type;
  int version,samples_per_scan, start_angle,end_angle, max_num_points_scan, watchdogtimeout, 
  num_layers;
  float range_min,range_max;
  num_layers = 0;
  bool watchdog, apply_correction = 0;
  if (!node->has_parameter("version"))
  {
    node->declare_parameter("version", version);
  }
  node->get_parameter("version", version);
  RCLCPP_INFO(node->get_logger(), "device name: %d", version);
  ...
  RCLCPP_INFO(node->get_logger(), "start_angle: %d", start_angle);
    ...
    if (!node->has_parameter("end_angle"))
  {
    node->declare_parameter("end_angle", end_angle);
  }
  node->get_parameter("end_angle", end_angle);

  RCLCPP_INFO(node->get_logger(), "end_angle: %d", end_angle);


    if (!node->has_parameter("range_min"))
  {
    node->declare_parameter("range_min", range_min);
  }
  node->get_parameter("range_min", range_min);

  RCLCPP_INFO(node->get_logger(), "range_min: %f", range_min);

      if (!node->has_parameter("range_max"))
  {
    node->declare_parameter("range_max", range_max);
  }
  node->get_parameter("range_max", range_max);

  RCLCPP_INFO(node->get_logger(), "range_max: %f", range_max);
  ...
  config->end_angle = node->get_parameter("end_angle").get_parameter_value().get<int>();
  config->version = node->get_parameter("version").get_parameter_value().get<int>();
  ...
  config->range_min = node->get_parameter("range_min").get_parameter_value().get<float>();
  config->range_max = node->get_parameter("range_max").get_parameter_value().get<float>();

  // RCLCPP_INFO(node->get_logger(), "start_angle: %d", config->start_angle);
  // RCLCPP_INFO(node->get_logger(), "end_angle: %d", config->end_angle);

  // RCLCPP_INFO(node->get_logger(), "I am here  %d", 1);

  OleiInterface olei_lidar(node);
  ...
  bool retrying = false;
  // RCLCPP_INFO(node->get_logger(), "I am here  %d", 2);
  while (rclcpp::ok())
  {
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 3);

    net_fail = false;
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 4);
    if (!olei_lidar.init(info,config,params,topic,frame_id,num_layers,version))
    {
    ...
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 5);
    if(!olei_lidar.start_transmission(net_mtx_,net_cv_,net_fail))
    {
      RCLCPP_ERROR(node->get_logger(), "Unable to start scan");
      return -1;
    }
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 6);
    retrying = true;
    // wait for condition variable
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 7);

    std::unique_lock<std::mutex> net_lock(*net_mtx_);
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 8);
    net_cv_->wait(net_lock, [] { return net_fail; });
    RCLCPP_ERROR(node->get_logger(), "Network failure");
    olei_lidar.terminate();
    // RCLCPP_INFO(node->get_logger(), "I am here  %d", 9);
  }
  std::cout << "I am here" << std::endl;
  // RCLCPP_INFO(node->get_logger(), "I am here  %d", 10);
  rclcpp::shutdown();
  t.join();
  t2.join();
  return 0;

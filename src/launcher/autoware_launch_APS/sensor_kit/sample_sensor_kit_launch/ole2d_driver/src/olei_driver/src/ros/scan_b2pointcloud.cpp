...
: rclcpp::Node("scan_b2pointcloud", options)
{
  ...
  pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_back", rclcpp::SensorDataQoS());
  ...
  while (rclcpp::ok(context) && alive_.load()) {
    ...
    if (subscription_count > 0) {
      if (!sub_.getSubscriber()) {
        ...
        sub_.subscribe(this, "/scan_b", qos.get_rmw_qos_profile());
      }

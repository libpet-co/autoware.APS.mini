...
Pipeline::Pipeline(std::shared_ptr<Writer<OleiPacket>> writer, std::shared_ptr<Reader<OleiPacket>> reader, 
...
{
  std::vector<std::unique_ptr<OleiPacket>> packets;
  while (running_)
  {     
...
{
  std::unique_ptr<OleiPacket> packet;
  while (running_)
  {
